---
description: Library for Time ("system" feature).
---
...
## Time.diff\_ns(prev: float) -> ns

Convenience function for getting the difference in nanoseconds between a previous timestamp (takes any units, default ns) and the current time. Shorthand for (Time.now\_ns() - prev).
...
## Time.from\_rfc2822(time: str) -> ms
...
## Time.from\_rfc3339(time: str) -> ms
...
## Time.to\_rfc2822(time: float) -> str
...
## Time.to\_rfc3339(time: float) -> str
